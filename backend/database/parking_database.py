...
class ParkingDatabase:
    ...
    def create_parking_lot(self, name: str, location: str, total_spaces: int, 
        ...
        """Create a new parking lot (basic)"""
        try:
    ...
    def create_parking_lot_with_details(self, name: str, location: str, 
                                       total_spaces: int, coordinates: Dict,
                                       address: str = None, 
                                       video_filename: str = None) -> Dict:
        """Create a new parking lot with full details"""
        try:
            lot_data = ParkingLotModel.create_lot(
                name=name,
                location=location,
    ...
    def update_lot_video(self, lot_id: str, video_filename: str) -> bool:
        """Update the video filename for a parking lot"""
        try:
            if self.parking_lots is not None:
                result = self.parking_lots.update_one(
                    {'lot_id': lot_id},
                    {
                        '$set': {
                            'video_filename': video_filename,
                ...
                logger.info(f"Updated video for lot {lot_id}: {video_filename}")
                return result.modified_count > 0
            return False
        except Exception as e:
            logger.error(f"Error updating lot video: {e}")
            return False
